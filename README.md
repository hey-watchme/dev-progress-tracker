...
### データベースビュー（テーブル）
| Title | Date | Commits | Repositories |
|-------|------|---------|--------------|
| Report 2025-11-28 | 2025-11-28 | 6 | app-ios, dev-progress-tracker |

### ページ本文（各レコードをクリック）
各レコードをクリックすると、改行・箇条書きで整形された詳細が表示されます：
...
**注意**: `Activities`プロパティは不要です。コミット詳細は各ページの本文（children）に改行・箇条書きで記録されます。
...
- **日付**（Date）
- **コミット数**（Number）
- **アクティブだったリポジトリ一覧**（Multi-select）

### ページ本文
- **コミットメッセージの詳細**（見出し・箇条書きで整形）
- **リポジトリごとにグループ化**
- **コミット作成者**
